...
public class RecordManager implements ChannelEventListener {
	...
	public boolean startRecord(String uid, Channel channel, String fileName) {
		...
		try {
			...
			// 保存开始录制时间
			files.setStarttime(new Date(System.currentTimeMillis()));
			//保存开始录制时频道ID
			files.setChannelid(recorder.getCid());
	...
	public boolean stopRecord(String uid) {
		...
		try {
			...
			files.setFilesize(new File(recorder.getAddr()).getTotalSpace());
			files.setPath(recorder.getAddr());
			files.setStoptime(new Date(System.currentTimeMillis()));
